        ...
        <div className="flex gap-2">
            <button 
              onClick={() => {
                ...
                const geminiKeys = typeof localStorage !== 'undefined' ? localStorage.getItem('gemini_api_keys') : null;
                const openaiKeys = typeof localStorage !== 'undefined' ? localStorage.getItem('openai_api_keys') : null;
                const anthropicKeys = typeof localStorage !== 'undefined' ? localStorage.getItem('anthropic_api_keys') : null;
                
                const summary = {
                  gemini: geminiKeys ? geminiKeys.split('\n').filter(k => k.trim()).length : 0,
                  openai: openaiKeys ? openaiKeys.split('\n').filter(k => k.trim()).length : 0,
                  anthropic: anthropicKeys ? anthropicKeys.split('\n').filter(k => k.trim()).length : 0,
                ...
                logger.info('🔑 API Kulcs Összegzés:', summary);
                logger.info(`📊 Összesen: ${Object.values(summary).reduce((a, b) => a + b, 0)} kulcs`);
              }} 
