...
export default function TeacherSessionManager({ library, onExit, onLibraryUpdate }: Props) {
  ...
  const getAllSessionHistory = async () => {
    try {
      // Try to get sessions from API with subject filter
      const response = await fetch(`/api/simple-api/sessions/list?subject=${currentSubject || 'all'}`);
      
      if (response.ok) {
        const data = await response.json();
        console.log('📊 API Session History:', data);
        ...
        return data.sessions.map((session: any) => ({
          ...
          averagePercentage: session.averagePercentage,
          performanceDistribution: session.performanceDistribution,
          isActive: session.isActive,
          createdAt: session.createdAt,
          summaries: [], // Will be populated from localStorage if available
          results: []    // Will be populated from localStorage if available
        }));
      }
    } catch (error) {
      console.warn('Could not load sessions from API, falling back to localStorage:', error);
    }
