...
export default function TeacherSessionManager({ library, onExit, onLibraryUpdate }: Props) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      {/* Storage Management Section - Prominent Display */}
      <div className="bg-gradient-to-r from-slate-50 to-slate-100 rounded-2xl p-6 mb-8 border border-slate-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="bg-slate-100 text-slate-700 w-12 h-12 flex items-center justify-center rounded-xl shadow-sm font-bold text-xl border border-slate-200">
              💾
            </div>
            <div>
              <h3 className="text-xl font-bold text-slate-800">Tárhely kezelés</h3>
              <p className="text-slate-600">Böngésző tárhely állapota és tisztítási lehetőségek</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="text-right">
              {(() => {
                ...
                const bgColor = usage.percentage > 80 ? 'bg-red-50 border-red-200' : usage.percentage > 60 ? 'bg-yellow-50 border-yellow-200' : 'bg-green-50 border-green-200';
                
                return (
                  <div className={`px-4 py-3 rounded-lg border-2 ${bgColor}`}>
                    ...
                    {cleanupEstimate.itemCount > 0 && (
                      <div className="text-xs text-slate-500 mt-1">
                        Törölhető: {cleanupEstimate.itemCount} elem ({cleanupEstimate.sizeKB}KB)
                      </div>
                    )}
                  </div>
                );
                  ...
                  <>
                    ...
                    {cleanupEstimate.itemCount > 0 && (
                      ...
                      >
                        ...
                        {usage.percentage > 80 ? '🚨 Azonnali tisztítás' : '🗑️ Tisztítás'}
                        {cleanupEstimate.sizeKB > 0 && ` (${cleanupEstimate.sizeKB}KB)`}
                      </button>
                    )}
                    
                    {/* Emergency Cleanup - Show when storage is critical */}
                    {usage.percentage > 90 && (
                      <button
                        onClick={() => {
                          if (confirm('🚨 VÉSZHELYZETI TISZTÍTÁS\n\nA tárhely kritikusan tele van!\n\nEz MINDEN tárolt adatot töröl (munkamenetek, beállítások, stb.)\n\n⚠️ EZ NEM VISSZAVONHATÓ!\n\nBiztosan folytatod?')) {
                            // Nuclear option - clear everything
                            localStorage.clear();
                            sessionStorage.clear();
                            alert('🚨 MINDEN ADAT TÖRÖLVE!\n\nA tárhely teljesen üres. Az oldal újratöltődik.');
                            window.location.reload();
                          }
                        }}
                        className="px-4 py-2 bg-red-700 hover:bg-red-800 text-white rounded-lg font-bold flex items-center gap-2 border border-red-700 animate-pulse"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="mt-4">
          {(() => {
            const usage = SafeStorage.getUsage();
            const color = usage.percentage > 80 ? 'bg-red-500' : usage.percentage > 60 ? 'bg-yellow-500' : 'bg-green-500';
