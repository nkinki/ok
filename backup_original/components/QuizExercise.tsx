...
const QuizExercise: React.FC<Props> = ({ content, onComplete, onNext }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl p-6 md:p-8 shadow-sm border border-gray-100 mb-6">
        ...
        {isMulti && <p className="text-xs text-brand-600 font-bold mb-4 uppercase tracking-wide">Több válasz is lehetséges</p>}
        ...
        {!isAnswered ? (
          <button
            ...
            className={`
              ...
              ${(selectedOption !== null && (!Array.isArray(selectedOption) || selectedOption.length > 0)) ? 'bg-brand-600 hover:bg-brand-700 transform hover:-translate-y-1' : 'bg-gray-300 cursor-not-allowed'}
            `}
          >
            Ellenőrzés
          </button>
          ...
          <button
            onClick={handleNext}
            className="bg-brand-600 hover:bg-brand-700 text-white px-8 py-3 rounded-full font-bold shadow-lg transition-all transform hover:-translate-y-1 flex items-center gap-2"
          >
            {currentIndex === content.questions.length - 1 ? "Befejezés" : "Következő kérdés"}
