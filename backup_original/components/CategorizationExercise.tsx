...
const CategorizationExercise: React.FC<Props> = ({ content, onComplete, onNext }) => {
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      {/* Unassigned Items (Word Bank) - Sticky Top */}
      <div 
        className={`
            bg-white p-4 rounded-xl border-2 border-brand-100 shadow-lg sticky top-0 z-20 transition-all mb-6
        ...
        {content.categories.map(cat => {
             ...
             return (
              <div key={cat.id} className="flex flex-col h-full">
                <div className="bg-brand-600 text-white p-3 rounded-t-xl font-bold text-center shadow-sm text-sm md:text-base">
                  {cat.name}
                </div>
                <div 
                    onDragOver={handleDragOver}
      ...
      {isSubmitted && (
         <div className="fixed bottom-6 left-0 w-full flex justify-center pointer-events-none z-30 gap-4">
             <button 
                ...
                className="pointer-events-auto bg-white text-brand-600 border-2 border-brand-200 px-6 py-3 rounded-full font-bold shadow-lg hover:bg-gray-50 transition-colors"
             >
                Újrakezdés
             </button>
             {onNext && (
                <button 
                    onClick={onNext}
                    className="pointer-events-auto bg-brand-600 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-brand-700 transition-colors flex items-center gap-2"
                >
                    Következő feladat
